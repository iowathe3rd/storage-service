...
export class FileService {
	...
	async upload(
		...
		try {
			const folderCandidate = await this.folderModel
				.getFolderById(createFileDto.folderId)
				.catch((e) => {
			...
			const fileCandidate = await db.file
				.findUnique({
					where: {
			...
			const { key: s3Key } = await this.s3Service
				.uploadFile(file, fileFullPath)
				.catch((e) => {
					throw new HttpException(
						{

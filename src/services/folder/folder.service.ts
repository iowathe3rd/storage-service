...
export class FolderService {
	...
	async create(data: CreateFolderDTO, userId: string): Promise<Folder | HttpException>  {
		try {
			const parentFolder = await db.folder.findUnique({
				...
				throw new HttpException({reason: "Parent folder not found", error: e}, HttpStatus.BAD_REQUEST);
			})
			...
			return await db.folder.create({data: folderToCreate}).catch(e=>{
				throw new HttpException({reason: "Something went wrong", error: e}, HttpStatus.BAD_GATEWAY)
			});
		} catch (e) {
			console.log(e);
	...
	async getFolder(id: string):  Promise<Folder | HttpException>  {
		try {
			return await db.folder.findUnique({
				...
				throw new HttpException({ reason: "Requested folder not found", error: e }, HttpStatus.BAD_REQUEST);
			});
		} catch (e) {
			console.log(e);
	...
	async updateFolder(dto: UpdateFolderDto, id: string): Promise<Folder | HttpException>  {
		try {
			const folderToUpdate = await db.folder.findUnique({
				where: {
					id: dto.id
				}
			}).catch(e=>{
	...
	async deleteFolder(id: string): Promise<object | HttpException> {
		try {
			...
			await this.s3Service.deleteFolder(folderToDelete.fullPath).catch(e=> {
